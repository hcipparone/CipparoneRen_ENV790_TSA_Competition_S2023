...
#ACF and PACF plots
par(mfrow=c(1,2))
ACF_Plot <- Acf(daily.demand$daily_mean_demand, lag = 40, plot = TRUE)
PACF_Plot <- Pacf(daily.demand$daily_mean_demand, lag = 40)
par(mfrow=c(1,1))
```
...
ts_training<-window(ts_demand_daily, start=2005, end=2009)


#Model 1: Seasonal naive on training data
SNAIVE_seas <- snaive(ts_training, h=365)

# Model 2:  SARIMA on training data
SARIMA_autofit <- auto.arima(ts_training)

#Model 3: STL + ETS model on training data
ETS_fit <-  stlf(ts_act_power_daily_train,h=365)


#Model 4: ARIMA with Fourier on training data (seasonal = False, K=c(2,12))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=FALSE, 
                             lambda=0,
...
#Model 5: ARIMA with Fourier on training data (seasonal = False, K=c(2,6))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=FALSE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 6: ARIMA with Fourier on training data (seasonal = TRUE, K=c(2,6))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=TRUE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 7: ARIMA with Fourier on training data (seasonal = TRUE, K=c(2,12))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=TRUE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 8: TBATS on training data
TBATS_fit <- tbats(ts_training)


#Model 9: Neural Network with Fourier on training data
...
MEAN_seas_2010 <- forecast(MEAN_seas, h=365)

#Model 2: Seasonal naive on original data
SNAIVE_seas_2010 <- forecast(SNAIVE_seas, h=1)

# Model 3:  SARIMA on original data
SARIMA_forecast_2010 <- forecast(object = SARIMA_autofit, h = 365)


autoplot(ts_demand_daily) +
  autolayer(MEAN_seas_2010, series="SEAS",PI=FALSE)+
  ylab("demand") 
...
```{r}
#Model 1: Arithmetic mean on original data
MEAN_seas_2011 <- meanf(y = ts_demand_daily, h = 365)
plot(MEAN_seas)
...
# Model 3:  SARIMA on original data
SARIMA_forecast_2011 <- forecast(object = SARIMA_autofit, h = 365)
...
```{r}
export(SARIMA_forecast_2011, file="./Competition copy/Output/submission.xlsx")
```
