...
MEAN_seas <- meanf(y = ts_training, h = 365)
...
#Model 6: ARIMA with Fourier on training data (seasonal = False, K=c(2,6))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=FALSE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 7: ARIMA with Fourier on training data (seasonal = TRUE, K=c(2,6))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=TRUE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 8: ARIMA with Fourier on training data (seasonal = TRUE, K=c(2,12))
ARIMA_Four_fit <- auto.arima(ts_training, 
                             seasonal=TRUE, 
                             lambda=0,
                             xreg=fourier(ts_act_power_daily_train, 
...
#Model 9: TBATS on training data
TBATS_fit <- tbats(ts_training)


#Model 10: Neural Network with Fourier on training data
...
SNAIVE_seas_2010 <- snaive(ts_demand_daily, h=1)
plot(SNAIVE_seas)

# Model 3:  SARIMA on original data
SARIMA_forecast_2010 <- forecast(object = SARIMA_autofit, h = 1)
