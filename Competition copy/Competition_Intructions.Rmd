...
daily.demand <- hourly.demand %>% 
  pivot_longer(cols=3:26,
               names_to="hourly.demand",
               values_to="demand") %>% 
  group_by(date) %>% 
  summarise(daily_mean_demand=mean(demand))
...
ts_demand_daily <- msts(daily.demand$daily_mean_demand, 
                           seasonal.periods =c(7,365.25), #Note that we might also try using just 7 instead of both. Suggested in Lesson 10
                           start=c(2005,1,1))
...
MEAN_seas <- meanf(y = ts_demand_daily, h = 30)
plot(MEAN_seas)

#Model 2: Seasonal naive on original data
SNAIVE_seas <- snaive(ts_demand_daily, h=30)
plot(SNAIVE_seas)

# Model 3:  SARIMA on original data
SARIMA_autofit <- auto.arima(ts_demand_daily)
...
```{r}
#Model 1: Arithmetic mean on original data
MEAN_seas_2010 <- meanf(y = ts_demand_daily, h = 1)
plot(MEAN_seas)
